function cost = attitude_project(Kp_test, Kd_test)
    % clc; clear; close all;
    ...
    [t, X] = ode45(@(t, X) spacecraft_dynamics(t, X, I, rho, v_orb, Cd, geom, Kp_test, Kd_test), tspan, X0, options);
    ...
    w_history = X(:, 5:7);
    dt = diff(t);
    q_penalty = sum(sum(q_err_history(1:end-1, :).^2, 2) .* dt);
    w_penalty = sum(sum(w_history(1:end-1, :).^2, 2) .* dt);
    cost = q_penalty + (1 * w_penalty);
end